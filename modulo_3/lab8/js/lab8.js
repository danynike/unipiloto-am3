...
$(document).ready(function(){
	function Validate(){};
  		Validate.prototype = {//forma de heredar  una funcion
  ...
  $('.nid-error').addClass('error');//adiciona una clase css
  $('.nid-error').hide();
...
$('.button').bind('click', function(event){//guarda los eventos de la pagina
			      ...
			      $('#nid').next().hide();
			      $('.nid-error').next().hide();
			      localStorage.setItem('nid', data);//localstorage el espacio es de 2MB y se puede ampliar a 5MB(no es recomendable) -- para almacenar informacion local cuando esta offline (no es volatil)
			    }
			    else{
			      $('#nid').next().show();
